...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${displayHours}:${minutes}:${seconds} ${amOrPm}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstMondayOffset = (startOfYear.getDay() + 6) % 7;
  const weekNumber = Math.floor((daysPassed + firstMondayOffset) / 7) + 1;
  return weekNumber;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      workSchedule.push(currentDate.toLocaleDateString('en-GB'));
